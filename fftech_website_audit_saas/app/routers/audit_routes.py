...
def run_audit_registered(data: RegisteredAuditRequest, request: Request, db: Session = Depends(db_session)):
    url = data.url
...
def audit_pdf(audit_id: int, db: Session = Depends(db_session)):
    ...
    return StreamingResponse(
        ...
        headers={'Content-Disposition': f'attachment; filename="FFTech_Audit_{audit_id}.pdf"'}
    )

@router.post("/generate-pdf")
def generate_pdf_direct(data: AuditRequest, db: Session = Depends(db_session)):
    """
    Instant PDF generation for open access users (no audit ID needed)
    """
    url = data.url
    ...
    try:
        ...
        pdf_bytes = build_pdf(payload, overall, cat_scores, url)
        
        return StreamingResponse(
            io.BytesIO(pdf_bytes),
            media_type="application/pdf",
            headers={
                "Content-Disposition": f'attachment; filename="FFTech_Audit_Report_{url.replace("https://", "").replace("/", "_")}.pdf"'
            }
        )
...
def audit_exports(audit_id: int, db: Session = Depends(db_session)):
    ...
    return StreamingResponse(
        ...
        headers={'Content-Disposition': f'attachment; filename="audit_{audit_id}_exports.zip"'}
    )
