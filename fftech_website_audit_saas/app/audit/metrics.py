...
# Metric registry IDs 1..200 (keys only, values are canonical keys)
EXECUTIVE = {
...
ONPAGE = {i: k for i, k in zip(range(41, 76), [
    ...
    'missing_structured_data','structured_data_errors','rich_snippet_warnings','missing_og_tags','long_urls',
    'uppercase_urls','non_seo_friendly_urls','too_many_internal_links','pages_without_incoming_links','orphan_pages',
    'broken_anchor_links','redirected_internal_links','nofollow_internal_links','link_depth_issues','external_links_count',
    'broken_external_links_onpage','anchor_text_issues'
])}
...
COMPETITOR = {i: k for i, k in zip(range(151, 168), [
    ...
    'competitor_security_score','competitive_gap_score','competitive_opportunity_heatmap','competitive_risk_heatmap','overall_competitive_rank'
])}
...
ALL_METRICS = {}
for group in [EXECUTIVE, OVERALL, CRAWLABILITY, ONPAGE, PERF, MOBILE_SECURITY_INTL, COMPETITOR, BROKEN_LINKS, OPPORTUNITIES]:
