...
from .crawler import crawl_site, fetch
...
async def analyze(url: str, competitors: List[str] | None = None) -> Dict[str, Any]:
    ...
    total = len(pages)
    status_counts = {k: 0 for k in [0, 200, 201, 204, 301, 302, 304, 400, 401, 403, 404, 410, 500, 502, 503, 504]}
    html_sizes = []
    for p in pages:
        s = p.get('status', 0)
    ...
    requests_per_page = 30  # placeholder
    ...
    for p in pages:
        ...
        l = html.lower()
        if '<title' not in l: missing_title += 1
        if 'meta name="description"' not in l and "meta name='description'" not in l: missing_meta_desc += 1
    ...
    metrics = {
        'http_2xx': http_2xx,'http_3xx': http_3xx,'http_4xx': http_4xx,'http_5xx': http_5xx,
        'total_crawled_pages': total,
        'missing_title': missing_title,'missing_meta_desc': missing_meta_desc,
        'total_page_size': int(avg_page_size),'requests_per_page': requests_per_page,
    }
    ...
    if competitors:
        async with httpx.AsyncClient(follow_redirects=True) as client:
            for cu in competitors[:5]:
                try:
                    r = await fetch(client, cu)
                    c_html_size = len(r.get('html','')) if r.get('html') else 0
                    c_status = r.get('status', 0)
                    c_health = max(0, 100 - (0 if 200 <= c_status < 300 else 40) - (c_html_size/500000*100))
                    comp_list.append({'url': cu,'status': c_status,'avg_html_size': c_html_size,'competitor_health_score': round(c_health,2)})
                except Exception:
                    comp_list.append({'url': cu,'status': 0,'avg_html_size': 0,'competitor_health_score': 0})

    return {'pages': pages,'metrics': metrics,'category_scores': {k: round(v,2) for k,v in category_scores.items()},'competitors': comp_list}