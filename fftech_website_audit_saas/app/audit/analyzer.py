...
async def analyze(url: str, competitors: List[str] | None = None) -> Dict[str, Any]:
    ...
    avg_page_size = int(sum(html_sizes)/len(html_sizes)) if html_sizes else 0
    ...
    for p in pages:
        ...
        if 'meta name="description"' not in low and "meta name='description'" not in low: missing_meta_desc += 1
