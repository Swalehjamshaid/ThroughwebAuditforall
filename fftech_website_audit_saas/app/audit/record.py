...
def export_graphs(audit_id: int, category_scores: dict, out_dir: str) -> str:
    ...
    plt.title('Category Scores'); plt.xticks(rotation=30, ha='right'); plt.tight_layout(); plt.savefig(path); plt.close()
    return path
...
def export_pptx(audit_id: int, png_chart_path: str, metrics: dict, out_dir: str, competitors: list | None = None) -> str:
    os.makedirs(out_dir, exist_ok=True)
    ...
    if competitors:
        comp_path = os.path.join(out_dir, f"audit_{audit_id}_competitors.png")
        labels = [ (c.get('url',''))[:20] for c in competitors ]
        vals = [ c.get('competitor_health_score',0) for c in competitors ]
        plt.figure(figsize=(8,3)); plt.bar(labels, vals, color='#f59e0b'); plt.title('Competitor Health'); plt.tight_layout(); plt.savefig(comp_path); plt.close()
        slide = prs.slides.add_slide(prs.slide_layouts[5])
        slide.shapes.add_picture(comp_path, Inches(1), Inches(1.2), width=Inches(8))
