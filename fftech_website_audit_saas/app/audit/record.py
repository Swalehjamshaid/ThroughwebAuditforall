...
def export_graphs(audit_id: int, category_scores: dict, out_dir: str) -> str:
    ...
    plt.title('Category Scores'); plt.xticks(rotation=30, ha='right'); plt.tight_layout(); plt.savefig(p); plt.close()
    return p
...
def export_xlsx(audit_id: int, metrics: dict, category_scores: dict, out_dir: str) -> str:
    ...
    with pd.ExcelWriter(p, engine='openpyxl') as w:
        ...
        pd.DataFrame([category_scores]).to_excel(w, index=False, sheet_name='CategoryScores')
    return p
