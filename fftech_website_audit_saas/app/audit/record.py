...
def render_dashboard_png(out_path: str, brand: str, url: str, category_scores: list, metrics_raw: dict):
    ...
    # Visual constants (local import safe if you have module-level constants)
    ...
    # Helper functions (repeat here to avoid cross-scope issues)
    def _safe_int(v):
        try:
            return int(v)
        except Exception:
    ...
    for label, ok in sec_items:
        ...
        # Rectangle with transform=ax3.transAxes (so 0..1 coordinates)
    ...
    # ✅ Fixed f-string: escaped newline inside a single, closed string
