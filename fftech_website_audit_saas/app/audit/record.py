...
def render_dashboard_png(out_path: str, brand: str, url: str, category_scores: list, metrics_raw: dict):
    ...
    - Compliance block (title/meta length vs thresholds)
    - Security matrix (XFO, CSP, HSTS, Cookie Secure/HttpOnly, Robots)
    """
    # Extract metrics
    ...
    for label, ok in sec_items:
        ...
        ax3.text(
            x0 + 0.06, y0 + 0.05, f"{label}: {'OK' if ok else 'Missing'}",
            va="center", fontsize=10, color=COLORS["fg"], transform=ax3.transAxes
        )
        y0 -= 0.13

    # ✅ Correctly closed f-string with an explicit newline character
...
def export_ppt(out_pptx: str, brand: str, url: str, grade: str, health_score: int,
    ...
    slide.placeholders[1].text = (
        f"{url}\n"
        f"Grade: {grade}  |  Health: {health_score}/100\n"
