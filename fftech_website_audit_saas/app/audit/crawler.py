...
async def crawl_site(start_url: str, max_pages: int = 50) -> List[Dict[str, Any]]:
    visited: Set[str] = set(); to_visit: List[str] = [start_url]; pages: List[Dict[str, Any]] = []
    parsed_root = urlparse(start_url); base_netloc = parsed_root.netloc
    async with httpx.AsyncClient(follow_redirects=True) as client:
        while to_visit and len(pages) < max_pages:
            url = to_visit.pop(0)
            if url in visited: continue
            visited.add(url)
            page = await fetch(client, url); pages.append(page)
            html = page.get('html', '')
            if not html: continue
            soup = BeautifulSoup(html, 'lxml')
            for a in soup.find_all('a', href=True):
                href = urljoin(url, a['href']); p = urlparse(href)
