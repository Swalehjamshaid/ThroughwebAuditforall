...
UA = {'User-Agent': 'FFTechAuditBot/1.0 (+https://fftech.ai)'}
...
async def crawl_site(start_url: str, max_pages: int = 40) -> List[Dict[str, Any]]:
    visited: Set[str] = set()
    ...
    async with httpx.AsyncClient(follow_redirects=True) as client:
        while queue and len(out) < max_pages:
            ...
            for a in soup.find_all('a', href=True):
                ...
                p = urlparse(href)
                if p.scheme in ('http','https') and p.netloc == base and href not in visited and href not in queue:
                    queue.append(href)
    return out