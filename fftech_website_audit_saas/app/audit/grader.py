...
def summarize_200_words(url: str, category_scores: Dict[str, int], top_issues: List[str]) -> str:
    parts = [
        ...
        f"Scores were: " + ", ".join([f"{k}: {v}/100" for k, v in category_scores.items()]) + ".",
    ]
    if top_issues:
        parts.append("Top issues: " + ", ".join(top_issues) + ".")
    else:
        parts.append("No critical issues were detected during the basic audit.")
    parts.append("The results suggest focusing on HTTPS, metadata quality, and image alt text for accessibility. Performance can be improved by minimizing payloads and enabling compression and caching.")
    txt = " ".join(parts)
    # Trim/extend near ~200 words (simple heuristic)
    return txt[:1200]
