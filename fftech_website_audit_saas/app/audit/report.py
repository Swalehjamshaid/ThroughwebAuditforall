...
LOGO_PATH = os.getenv('BRAND_LOGO_PATH', os.path.join(PKG_DIR, 'static', 'img', 'logo.png'))
...
def build_pdf(audit_id: int, url: str, overall_score: float, grade: str, category_scores: dict, metrics: dict, out_dir: str) -> str:
    ...
    def header(page_title: str):
        try:
            c.drawImage(LOGO_PATH, 2*cm, height-3*cm, width=3*cm, preserveAspectRatio=True, mask='auto')
        except Exception:
            pass
    ...
    footer('Overall health depends on weighted category scores; fix weak areas first.')
    c.showPage()
    ...
    footer('Reduce 4xx/5xx and redirects; repair broken links and ensure clean crawl paths.')
    c.showPage()

    header('On-Page SEO')
    c.setFont('Helvetica', 11)
    c.drawString(2*cm, height-5*cm, f"Missing Titles: {metrics.get('missing_title',0)} | Missing Meta: {metrics.get('missing_meta_desc',0)}")
    footer('Tighten titles and meta; one H1; relevant alt text; add structured data.')
    c.showPage()
    ...
    footer('Improve LCP/CLS via image optimization, caching, and reducing render-blocking resources.')
    c.showPage()

    header('Mobile, Security & Opportunities')
    c.setFont('Helvetica', 11)
    c.drawString(2*cm, height-5*cm, 'Mobile-friendly and HTTPS as baseline; set up monitoring and quick wins.')
