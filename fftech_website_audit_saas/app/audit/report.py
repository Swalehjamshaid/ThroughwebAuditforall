...
def build_pdf(audit_id: int, url: str, overall_score: float, grade: str, category_scores: dict, metrics: dict, out_dir: str) -> str:
    ...
    footer('Focus on weak categories first to lift overall grade.')
    c.showPage()
    ...
    footer('Reduce 4xx/5xx, address redirect chains and ensure clean sitemap/robots.')
    c.showPage()
    ...
    footer('Improve titles/meta, H1 consistency, alt text and structured data for rich results.')
    c.showPage()
    ...
    footer('Optimize images, caching and render-blocking to improve LCP/CLS/TBT.')
    c.showPage()

    # Page 5 – Mobile, Security & Opportunities
    header('Mobile, Security & Opportunities')
    c.setFont('Helvetica', 11)
    c.drawString(2*cm, height-5*cm, 'Mobile baseline & HTTPS assumed; quick wins and roadmap recommended.')
    footer('Prioritize mobile CWV & HTTPS, then schedule continuous monitoring.')
    c.showPage()
