...
class Audit(Base):
    ...
    category_scores_json = Column(Text, nullable=True)    # stores list[{name,score}] as JSON
    metrics_json         = Column(Text, nullable=True)    # stores metrics list as JSON
    created_at           = Column(DateTime(timezone=True), server_default=func.now())
...
class Subscription(Base):
    ...
    plan                  = Column(String(32), default="free")  # free / paid
    active                = Column(Boolean, default=True)
    audits_used           = Column(Integer, default=0)
    # Scheduler columns (your main.py adds IF NOT EXISTS at startup; defined here for clean schema)
