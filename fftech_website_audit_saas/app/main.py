...
async def engine_page(request: Request):
    return templates.TemplateResponse('engine.html', {
        'request': request,
        'title': 'Run Engine (Graphical Output)',
    })
...
async def engine_run(
    ...
    RUNS[run_id] = {
        'source': 'engine',
        'target': target,
        'produced_path': str(produced_path) if produced_path else None,
...
async def run(
    ...
    if callable(grade_all) and input_path:
        try:
            grade_all(input_path=input_path, config={}, logger=logger)  # type: ignore
