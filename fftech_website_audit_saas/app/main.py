...
# ------------------------------------------------------------------------------
# FastAPI app, templates, global Jinja context
# ------------------------------------------------------------------------------
...
# Provide reusable globals to every template
...
def ready() -> JSONResponse:
    # simple readiness check against DB
    try:
        with engine.connect() as conn:
...
def _robust_audit(url: str) -> Tuple[str, Dict[str, Any]]:
    """
    Attempt multiple URL variants; return the first successful result or fallback.
    """
    ...
    for candidate in _url_variants(base):
        try:
            ...
            # accept if any category produced a positive score
...
@app.get("/auth/verify")
async def verify(request: Request, token: str, db: Session = Depends(get_db), user: Optional[User] = Depends(get_current_user)):
    try:
        data = decode_token(token)
...
async def audit_detail(
    ...
    # Trend from last 12 audits for this website
    history = (
        db.query(Audit)
