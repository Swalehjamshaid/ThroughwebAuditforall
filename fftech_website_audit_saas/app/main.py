...
def _robust_audit(url: str) -> tuple[str, dict]:
    ...
    for candidate in url_variants(base):
        try:
...
def _send_magic_login_email(to_email: str, token: str) -> bool:
    """
    Passwordless login link via email using SMTP. Avoid multi-line strings: build via join.
    """
    ...
    parts = [
        f"<h3>{UI_BRAND_NAME} — Magic Login</h3>",
        "<p>Hello!</p>",
        "<p>Click the secure link below to log in:</p>",
        f"<p><a href=\"{login_link}\" target=\"_blank\" rel=\"noopener noreferrer\">{login_link}</a></p>",
