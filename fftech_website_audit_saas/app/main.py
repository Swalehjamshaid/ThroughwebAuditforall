...
#!/usr/bin/env python3
"""
Centralized FastAPI main: exposes minimal HTML endpoints and integrates modular services.
- Provides `app` for uvicorn (ASGI).
- Renders HTML using Jinja2 templates.
- Generates graphs from report data and optional PDF outputs.
- Optional SessionMiddleware: auto-disabled if itsdangerous is not installed.
"""
...
from .services.config import setup_logging, DEFAULT_OUTPUT_DIR, SECRET_KEY
...
# Try to enable sessions only if itsdangerous is installed
try:
...
@app.get('/health')
async def health():
    return {'status': 'ok', 'sessions': HAS_SESSIONS}
