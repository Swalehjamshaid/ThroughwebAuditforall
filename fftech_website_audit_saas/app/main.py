...
async def verify(request: Request, token: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if u:
            u.verified = True; db.commit(); success = True
...
async def dashboard(request: Request, db: Session = Depends(get_db)):
    ...
    last_audits = db.query(Audit).filter(Audit.user_id == current_user.id).order_by(Audit.created_at.desc()).limit(5).all()
    if last_audits:
...
async def new_audit_post(request: Request, url: str = Form(...), enable_schedule: str = Form(None), db: Session = Depends(get_db)):
    ...
    if enable_schedule and hasattr(sub, 'email_schedule_enabled'):
        sub.email_schedule_enabled = True; db.commit()
...
async def run_audit(website_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    if not w:
        return RedirectResponse('/dashboard', status_code=303)
    try: res = run_basic_checks(w.url)
    except Exception: return RedirectResponse('/dashboard', status_code=303)
    category_scores_dict = res['category_scores']
    overall = compute_overall(category_scores_dict)
    grade = grade_from_score(overall)
    exec_summary = summarize_200_words(w.url, category_scores_dict, res['top_issues'])
    category_scores_list = [{'name': k, 'score': int(v)} for k, v in category_scores_dict.items()]
    audit = Audit(user_id=current_user.id, website_id=w.id, health_score=int(overall), grade=grade, exec_summary=exec_summary, category_scores_json=json.dumps(category_scores_list), metrics_json=json.dumps(res['metrics']))
...
async def audit_detail(website_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    if not w or not a: return RedirectResponse('/dashboard', status_code=303)
    ...
    data = {'request': request, 'UI_BRAND_NAME': UI_BRAND_NAME, 'user': current_user, 'website': w,
            'audit': {'created_at': a.created_at, 'grade': a.grade, 'health_score': a.health_score, 'exec_summary': a.exec_summary, 'category_scores': category_scores, 'metrics': metrics}}
...
async def schedule_get(request: Request, db: Session = Depends(get_db)):
    ...
    sub = db.query(Subscription).filter(Subscription.user_id == current_user.id).first()
    schedule = {'daily_time': getattr(sub,'daily_time','09:00'), 'timezone': getattr(sub,'timezone','UTC'), 'enabled': getattr(sub,'email_schedule_enabled', False)}
    return templates.TemplateResponse('dashboard.html', {'request': request, 'UI_BRAND_NAME': UI_BRAND_NAME, 'user': current_user,
        'websites': db.query(Website).filter(Website.user_id == current_user.id).all(), 'trend': {'labels': [], 'values': []}, 'summary': {'grade': 'A', 'health_score': 88}, 'schedule': schedule})
...
async def schedule_post(request: Request, daily_time: str = Form(...), timezone: str = Form(...), enabled: str = Form(None), db: Session = Depends(get_db)):
    ...
    sub = db.query(Subscription).filter(Subscription.user_id == current_user.id).first()
    if not sub:
        sub = Subscription(user_id=current_user.id, plan='free', active=True, audits_used=0); db.add(sub); db.commit(); db.refresh(sub)
    if hasattr(sub, 'daily_time'): sub.daily_time = daily_time
    if hasattr(sub, 'timezone'): sub.timezone = timezone
    if hasattr(sub, 'email_schedule_enabled'): sub.email_schedule_enabled = bool(enabled)
    db.commit(); return RedirectResponse('/dashboard', status_code=303)
...
async def admin_dashboard(request: Request, db: Session = Depends(get_db)):
    global current_user
    if not current_user or not current_user.is_admin: return RedirectResponse('/admin/login', status_code=303)
    ...
    return templates.TemplateResponse('dashboard.html', {'request': request, 'UI_BRAND_NAME': UI_BRAND_NAME, 'user': current_user, 'websites': websites, 'trend': {'labels': [], 'values': []}, 'summary': {'grade': 'A', 'health_score': 88}, 'schedule': {'daily_time': '09:00', 'timezone': 'UTC', 'enabled': False}, 'admin_users': users, 'admin_audits': audits})

# email scheduler

def _send_report_email(to_email: str, subject: str, html_body: str) -> bool:
    if not (SMTP_HOST and SMTP_USER and SMTP_PASSWORD): return False
    msg = MIMEMultipart('alternative'); msg['Subject'] = subject; msg['From'] = SMTP_USER; msg['To'] = to_email
    msg.attach(MIMEText(html_body, 'html'))
    try:
        with smtplib.SMTP(SMTP_HOST, SMTP_PORT) as server:
            server.starttls(); server.login(SMTP_USER, SMTP_PASSWORD); server.sendmail(SMTP_USER, [to_email], msg.as_string())
        return True
...
async def _daily_scheduler_loop():
    while True:
        try:
            db = SessionLocal(); subs = db.query(Subscription).filter(Subscription.active == True).all(); now_utc = datetime.utcnow()
            for sub in subs:
                if not getattr(sub, 'email_schedule_enabled', False): continue
                try: tz = ZoneInfo(getattr(sub,'timezone','UTC') or 'UTC')
                except Exception: tz = ZoneInfo('UTC')
                local_now = now_utc.replace(tzinfo=ZoneInfo('UTC')).astimezone(tz)
                if local_now.strftime('%H:%M') != getattr(sub,'daily_time','09:00') or not getattr(sub,'daily_time','09:00'): continue
                user = db.query(User).filter(User.id == sub.user_id).first()
                if not user or not getattr(user,'verified',False): continue
                websites = db.query(Website).filter(Website.user_id == user.id).all()
                lines = [f"<h3>Daily Website Audit Summary – {UI_BRAND_NAME}</h3>", f"<p>Hello, {user.email}!</p>", "<p>Here is your daily summary. Download certified PDFs via links below.</p>"]
                for w in websites:
                    last = db.query(Audit).filter(Audit.website_id == w.id).order_by(Audit.created_at.desc()).first()
                    if not last: lines.append(f"<p><b>{w.url}</b>: No audits yet.</p>"); continue
                    pdf_link = f"{BASE_URL}/report/pdf/{w.id}"
                    lines.append(f"<p><b>{w.url}</b>: Grade <b>{last.grade}</b>, Health <b>{last.health_score}</b>/100 (<a href='{pdf_link}' target='_blank'>Download Certified Report</a>)</p>")
                ...
                html = "
".join(lines); _send_report_email(user.email, f"{UI_BRAND_NAME} – Daily Website Audit Summary", html)
            db.close()
        except Exception: pass
