...
# Optional local fallback using reportlab (if available)
...
def fallback_pdf(rows: List[Dict[str, Any]], graphs: List[Path], output_dir: Path, logger) -> Optional[Path]:
    ...
    c.drawString(2*cm, height-2*cm, 'FF Tech AI Website Audit')
    ...
    for g in graphs[:6]:
        c.showPage()
        c.drawImage(str(g), 2*cm, 4*cm, width=width-4*cm, height=height-8*cm, preserveAspectRatio=True, mask='auto')
        c.setFont('Helvetica', 10)
        c.drawString(2*cm, 2.5*cm, f"Figure: {g.name}")

    c.save()
    logger.info('Fallback PDF created at: %s', pdf_path)
    return pdf_path
