...
"""
FF Tech AI Website Audit SaaS - Single File Backend & Web UI (Pydantic v2-ready)

Features:
- Open Access Audit (no login/history)
- Registered Access (passwordless email magic link)
- Free vs Pro plans (free limit: 10 audits; pro enables scheduling & long-term history)
- Audit history & PDF reporting (5 pages, branded visuals with ReportLab)
- 200-metric framework (subset computed in quick audit; transparent placeholders)
- API-first & frontend-agnostic + single-page HTML UI (Chart.js via CDN)
- Background scheduler for continuous monitoring & scheduled email PDF (SMTP optional)
- Railway-ready: uses PORT & DATABASE_URL + /health endpoint
...
try:
    ...
    PDF_ENABLED = False  # APIs & UI still run if reportlab not installed
...
# Plan & limits
FREE_AUDIT_LIMIT = 10
FREE_HISTORY_LIMIT = 10  # free users keep last 10 audits

# ---------- FastAPI App ----------
app = FastAPI(title=APP_NAME, version="1.3.0", description="API-driven, frontend-agnostic Website Audit SaaS by FF Tech")
...
class AuditEngine:
    ...
    def compute_metrics(self) -> Dict[int, Dict[str, Any]]:
        ...
        rb_js_sync = len(self.resources_js)  # heuristic
        ...
        m[168] = {"value": m.get(27, {"value":0})["value"] + m.get(28, {"value":0})["value"], "detail": "Total broken links (sample)"}
        m[169] = {"value": m.get(27, {"value":0})["value"], "detail": "Internal broken links"}
        m[170] = {"value": m.get(28, {"value":0})["value"], "detail": "External broken links"}
        ...
        # Ensure ALL metrics 1..200 exist (transparent placeholders for the rest)
...
def report_open_pdf(req: AuditRequest):
    ...
    eng = AuditEngine(req.url)
    metrics = eng.compute_metrics()
    # Transient Audit-like object
    a = Audit(id=0, user_id=None, url=req.url, metrics_json=json.dumps(metrics), score=metrics[1]["value"], grade=metrics[2]["value"])
    pdf = build_pdf_report(a, metrics)
    return StreamingResponse(io.BytesIO(pdf), media_type="application/pdf",
