...
<body><main class='p-6'>
<h1>FF Tech — Audit Platform</h1>
<form id='audit-form'><input id='website-url' type='url' placeholder='https://example.com' required>
<button type='submit'>Run Audit</button></form>
<section id='summary' style='display:none'><pre id='out'></pre></section>
<script>
document.getElementById('audit-form').addEventListener('submit',async(e)=>{e.preventDefault();
 const url=document.getElementById('website-url').value; const res=await fetch('/audit?url='+encodeURIComponent(url));
 const payload=await res.json(); document.getElementById('summary').style.display='block';
