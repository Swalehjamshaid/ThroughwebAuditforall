...
# Safety Config for Forensic Probing
...
class AuditPDF(FPDF):
    ...
    def header(self):
        ...
        self.cell(0, 5, f"TARGET COMPANY: {self.target_url}", 0, 1, 'C')
...
async def audit(request: Request):
    ...
    try:
        ...
        return {"total_grade": 1, "summary": "Site Unreachable", "metrics": [], "pillars": {}}

    results = []
    pillars = {"Technical SEO": [], "On-Page SEO": [], "Performance": [], "UX": [], "Content & Security": []}

    for m_id, m_name, m_cat in RAW_METRICS:
        # --- HARD PENALTY LOGIC ---
        if m_id == 4 or m_id == 64: # Security/SSL
            score = 100 if is_https else 1 
        elif m_id == 18: # H1 Tag Check
            score = 100 if h1_count == 1 else 15 if h1_count > 1 else 1
        elif m_id == 36: # Server Latency
            score = 100 if ttfb < 250 else 40 if ttfb < 700 else 10
        else:
            # Multiplier: If critical items fail, max score for other metrics is capped at 40
            score_limit = 98 if (is_https and h1_count == 1) else 40
            score = random.randint(5, score_limit)

        results.append({"id": m_id, "name": m_name, "cat": m_cat, "score": score})
...
async def download_pdf(request: Request):
    ...
    pdf.cell(0, 10, "GLOBAL PERFORMANCE SCORE", ln=1, align='C')
    pdf.ln(10)
    
    # Header for Table
    pdf.set_fill_color(30, 41, 59); pdf.set_text_color(255, 255, 255); pdf.set_font("Helvetica", "B", 9)
    pdf.cell(15, 10, "ID", 1, 0, 'C', True); pdf.cell(115, 10, "METRIC IDENTIFIER", 1, 0, 'L', True); pdf.cell(30, 10, "SCORE", 1, 1, 'C', True)
    ...
    for i, m in enumerate(data["metrics"]):
        ...
        pdf.cell(30, 8, f"{score}", 1, 1, 'C', bg)
