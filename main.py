...
# ==================== STRICT & REALISTIC SCORING (Google/Lighthouse-like) ====================
...
# ==================== BROWSER AUDIT (Robust with Stealth) ====================
async def browser_audit(url: str, mobile: bool = False):
    ...
    try:
        async with async_playwright() as p:
            ...
            await asyncio.sleep(3)  # Allow dynamic content & paints
            ...
            perf = await page.evaluate("""
                () => {
                    ...
                    const lcp = lcps[lcps.length - 1]?.startTime || 9999;
                    return {
                        fcp,
                        lcp,
                        cls: shifts,
...
async def audit(request: Request):
    try:
        ...
        # Blocked or failed audit → low score + message
        if perf["lcp"] > 9000:
            return {
                "url": url,
                "total_grade": 35,
                ...
                "summary": "Audit blocked (anti-bot) or timeout. Estimated low score.",
        ...
        scores = {
            ...
            "Accessibility Basics": score_bool_strict(bool(soup.find_all(attrs={"aria-label": True}))),
            ...
            "Secure Cookies": score_bool_strict("set-cookie" in headers and "secure" in headers.get("set-cookie", "").lower()),
            "No Mixed Content": score_bool_strict(url.startswith("https://")),
        }
...
async def download_pdf(request: Request):
    try:
        ...
        pdf.cell(0, 10, "66+ DETAILED METRICS", ln=1)
