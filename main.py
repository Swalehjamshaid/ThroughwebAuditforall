...
# web_audit.py
# Single-file, comprehensive web analysis engine:
# - FastAPI + embedded HTML UI
# - Real audit via Google PSI + DOM/headers + optional Playwright SPA rendering
# - 300+ metrics scoring, weighted pillars, multi-page ReportLab PDF (≥300-word roadmap)
# - Distinct scores for different sites (e.g., Apple vs Haier)
...
# Optional Playwright (SPA rendering / lab metrics)
try:
...
logger = logging.getLogger("WORLD_AUDIT")

# ==================== APP ====================
app = FastAPI(title="World's Most Comprehensive Web Analysis")
app.add_middleware(
    CORSMiddleware,
...
<header class="border-b border-slate-800">
  <div class="max-w-7xl mx-auto px-6 py-5 flex justify-between items-center">
    <h1 class="text-2xl font-bold text-emerald-400">FF TECH ELITE <span class="text-slate-400">v3</span></h1>
...
<section class="max-w-7xl mx-auto px-6 py-10">
  <div class="card rounded-xl p-8 glow">
    <div class="grid md:grid-cols-4 gap-4">
      <input id="urlInput" type="text" placeholder="example.com"
        class="col-span-2 px-4 py-3 rounded bg-slate-800 border border-slate-700 focus:outline-none" />
      <select id="modeInput" class="px-4 py-3 rounded bg-slate-800 border border-slate-700">
        <option value="desktop">Desktop Audit</option>
        <option value="mobile">Mobile Audit</option>
      </select>
      <button onclick="runAudit()" class="bg-emerald-500 hover:bg-emerald-600 text-black font-semibold rounded px-6 py-3">
        Run Audit
      </button>
    </div>
  </div>
...
<div id="loader" class="hidden flex flex-col items-center py-16">
  ...
  <p class="mt-4 text-slate-400">Running Lighthouse + DOM + Security + UX audit…</p>
</div>
...
<section id="results" class="hidden max-w-7xl mx-auto px-6 pb-24 space-y-12">
  ...
  <div class="grid md:grid-cols-5 gap-6">
    <div class="md:col-span-2 card rounded-xl p-8 text-center glow">
      ...
      <p id="auditedUrl" class="text-sm text-slate-500 mt-2"></p>
  ...
  <div class="card rounded-xl p-6">
    <h3 class="font-semibold mb-4">Core Web Vitals & Performance</h3>
  ...
  <div class="text-center">
    <button onclick="downloadPDF()" class="bg-emerald-500 hover:bg-emerald-600 text-black font-semibold px-10 py-4 rounded-xl">
...
function renderResults(data) {
  ...
  const p = document.getElementById("pillarScores"); p.innerHTML = "";
  ...
  const perf = document.getElementById("perfGrid"); perf.innerHTML = "";
  ...
  const tbody = document.getElementById("metricsTable"); tbody.innerHTML = "";
...
# Domain rewrite per your requirement: haier.pk variants => http://www.haier.com.pk/#/login
...
CRITICAL_METRICS = {
    "Performance": [
        ...
        "Cumulative Layout Shift (CLS)", "Interaction to Next Paint (INP)", "Speed Index", "Total Blocking Time (TBT)"
    ],
    "SEO": ["Page Title (Length &amp; Quality)", "Meta Description (Length &amp; Quality)", "H1 Tag Unique &amp; Present", "Canonical Tag Present", "Structured Data (Schema.org)"],
    "UX": ["Viewport Meta Tag", "Mobile-Friendly (PSI Viewport)", "Color Contrast Ratio", "Readable Font Sizes"],
    "Security": ["HTTPS Enforced", "HSTS Header", "Content-Security-Policy Header", "X-Frame-Options Header", "No Mixed Content"]
}
...
def apply_domain_rewrite(url: str) -> str:
    try:
        ...
        return DOMAIN_REWRITE_RULES.get(host, url)
    except Exception:
        return url

def normalize_url(raw_url: str) -> str:
    """Normalize & preserve fragment; add https:// if missing; then apply rewrite rules."""
    ...
    return apply_domain_rewrite(normalized)
    ...
    async with async_playwright() as p:
        ...
        metrics_js = await page.evaluate(
            """() => {
                ...
                const resourceNames = resources.map r => r.name || '' // intentionally left minimal
                return {
                    fcp: Math.round(fcp),
                    lcp: Math.round(lcp),
                    totalBytes,
...
def build_seo_audit(final_url: str, soup: BeautifulSoup, audit: Dict[str, Any], psi: Dict[str, Any]) -> Dict[str, Any]:
    ...
    off_page: Dict[str, Any] = {"message": "Connect an SEO API (Semrush/Ahrefs/GSC) to populate Off‑Page & Rankings."}
    ...
    return {
        ...
        "seo_score": psi.get("psi_performance_percent") or 0,
        "critical_issues": sum(1 for i in issues if i["priority"] == "red-flag"),
        "minor_issues": sum(1 for i in issues if i["priority"] != "red-flag"),
        "overview_text": "Automated site audit combining Lighthouse (PSI), Playwright (optional), DOM & headers.",
...
def _expand_roadmap_to_300_words(html_text: str, psi: Dict[str, Any]) -> str:
    ...
    if len((plain or "").split()) >= 300:
        return base
    add_lines = [
        "<br/><br/><b>Action Plan:</b>",
        "- Prioritize Core Web Vitals: reduce LCP (optimize hero assets, preload key fonts), drop render-blockers.",
        "- Improve FCP by inlining critical CSS and deferring non-critical scripts/styles.",
        "- Lower TBT & INP: code-split heavy JS, lazy load expensive widgets, minimize third-party tags.",
        "- Prevent CLS by reserving media/ads space, using aspect-ratio, and font-display: swap.",
        "- Optimize caching: long-lived cache-control for static assets; enable Brotli/GZIP; adopt HTTP/2.",
        "- Strengthen SEO: unique titles (50–60 chars), meta descriptions (100–130 chars), single H1, H2/H3 hierarchy.",
        "- Add JSON-LD structured data (Organization, Product, Breadcrumb, Sitelinks).",
        "- Security headers: add HSTS, CSP, X-Frame-Options, X-Content-Type-Options, Referrer-Policy, Permissions-Policy.",
        "- Fix mixed content; ensure cookies have Secure/HttpOnly/SameSite attributes.",
        "- Mobile UX: viewport meta, tap target spacing, readable font sizes, accessible contrast.",
        "- Internal linking: connect hub pages, reduce orphan pages, enrich anchor text.",
        "- Monitoring: enforce budgets, CI checks, Lighthouse CI; track CWV via CrUX/PSI.",
    ]
    base += "<br/>" + "<br/>".join(add_lines)
    while len(BeautifulSoup(base, "html.parser").get_text(" ").split()) < 300:
        base += "<br/>Iterate weekly: measure, remediate, retest across CWV, SEO, accessibility, and security."
    return base
...
async def audit(request: Request):
    ...
    # 2) Playwright lab OR requests fallback
...
async def download_pdf(request: Request):
    ...
    roadmap_long = _expand_roadmap_to_300_words(roadmap_html, perf)
    ...
    # Keyword Rankings (placeholder unless integrated)
