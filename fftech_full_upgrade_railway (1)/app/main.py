        ...
        try:
            ...
            for sub in subs:
                if not getattr(sub, 'email_schedule_enabled', False):
                    continue
                tz_name = getattr(sub, 'timezone', 'UTC') or 'UTC'; daily_time = getattr(sub, 'daily_time', '09:00') or '09:00'
                try: tz = ZoneInfo(tz_name)
                except Exception: tz = ZoneInfo('UTC')
                local_now = now_utc.replace(tzinfo=ZoneInfo('UTC')).astimezone(tz); hhmm_now = local_now.strftime("%H:%M")
                if hhmm_now != daily_time: continue
                ...
                for w in websites:
                    ...
                    if not last:
                        lines.append(f"<p><b>{w.url}</b>: No audits yet.</p>"); continue
