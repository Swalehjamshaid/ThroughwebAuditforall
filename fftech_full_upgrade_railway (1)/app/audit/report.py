...
# Radar via matplotlib (image embedded in PDF)
...
def _as_dict(category_scores):
    """Accept list [{'name','score'}] or dict and return dict."""
...
def render_pdf(path: str, brand: str, domain: str, grade: str, health_score: int, category_scores, exec_summary: str):
    ...
    para = Paragraph(exec_summary, style)
    frame.addFromList([para], c)
    c.showPage()

    # Page 3: Radar Chart + Category Scores
    ...
    for name, score in cat_dict.items():
        c.drawString(20, y-150, f"{name}: {score}")
        y -= 12
    c.showPage()
    try:
        os.remove(radar_png)
    ...
    # Page 4: Key Findings & Priority Fixes
    c.setFont('Helvetica-Bold', 16); c.drawString(20, h-40, 'Key Findings & Recommendations')
    c.setFont('Helvetica', 11)
    lines = [
        'Optimize LCP/FCP; eliminate render-blocking CSS/JS.',
        'Repair broken links; ensure sitemap & canonical consistency.',
        'Implement CSP + HSTS; fix mixed content; enforce HTTPS.',
        'Improve mobile tap targets & font sizes; check viewport settings.'
    ]
    y = h-70
    for ln in lines:
        c.drawString(20, y, f"• {ln}"); y -= 16
    c.showPage()

    # Page 5: Category Scores Snapshot
    c.setFont('Helvetica-Bold', 16); c.drawString(20, h-40, 'Category Scores Snapshot')
    c.setFont('Helvetica', 10); y = h-70
    for name, score in cat_dict.items():
        c.drawString(20, y, f"{name}: {score}%")
        y -= 12
        if y < 60:
