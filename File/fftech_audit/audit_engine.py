...
def safe_request(url: str, timeout: int = 10) -> Tuple[int, bytes, float, Dict[str, str]]:
    ...
    try:
        ...
        return resp.status_code, (resp.content or b""), time.time()-t0, dict(resp.headers or {})
    except Exception:
...
def safe_search(pattern: str, text: str, flags=0):
    try: return re.search(pattern, text, flags)
...
def register_metrics():
    sections = {
        ...
        "C": ["HTTP 2xx","HTTP 3xx","HTTP 4xx","HTTP 5xx","Broken internal links","Broken external links","robots.txt","Sitemap presence",
...
class AuditEngine:
    ...
    def compute_metrics(self) -> Dict[int, Dict[str, Any]]:
        ...
        meta_desc_match = safe_search(r'<meta[^>]+name=["\']description["\'][^>]*content=["\']([^"\']+)["\']', self.html, flags=re.IGNORECASE)
        ...
        canonical_match = safe_search(r'<link[^>]+rel=["\']canonical["\'][^>]*href=["\']([^"\']+)["\']', self.html, flags=re.IGNORECASE)
        ...
        desc_ok  = meta_desc and 50 <= len(meta_desc) <= 165
        if title_ok: strengths.append("Title length optimal")
        if desc_ok:  strengths.append("Meta description optimal")
        ...
        for pid in range(1, 201):
            if pid not in m: m[pid] = {"value": "N/A", "detail": "Not computed in quick audit"}
        return m
    ...
    def executive_summary(self, metrics: Dict[int, Dict[str, Any]]) -> str:
        ...
        return (f"{APP_BRAND} audited {self.url} across crawlability, on-page SEO, performance, mobile, and security. "
