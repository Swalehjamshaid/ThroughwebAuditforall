...
# Try requests; fallback to urllib (Railway supports requests)
try:
    import requests
except Exception:
    requests = None
...
# === Metric mapping (IDs 1..200) from your spec ===
A = [
...
for category, items in [
    ("Executive Summary & Grading", A),
    ("Overall Site Health", B),
    ("Crawlability & Indexation", C),
    ("On-page SEO", D),
    ("Performance & Technical", E),
    ("Mobile, Security & International", F),
    ("Competitor Analysis", G),
    ("Broken Links Intelligence", H),
    ("Opportunities, Growth & ROI", I),
]:
    for name in items:
...
def canonical_origin(url: str) -> str:
    u = urlparse(url); scheme = u.scheme or 'https'; host = u.netloc or u.path
...
class FetchResult:
    def __init__(self, status: int, headers: Dict[str,str], content: bytes, elapsed_ms: int, url: str):
        self.status = status; self.headers = headers; self.content = content; self.elapsed_ms = elapsed_ms; self.url = url

def fetch(url: str, timeout: float = 10.0) -> FetchResult:
    start = time.time(); headers = {}; content = b''; status = 0; final_url = url
    if requests:
        try:
            r = requests.get(url, headers={'User-Agent': USER_AGENT}, timeout=timeout, allow_redirects=True)
            status = r.status_code; headers = {k.lower(): v for k, v in r.headers.items()}
            content = r.content or b''; final_url = str(r.url)
    ...
    if status == 0:
        ...
        try:
            with urllib.request.urlopen(req, context=ctx, timeout=timeout) as resp:
                ...
                content = resp.read() or b''; final_url = resp.geturl()
...
def compute_category_security(sec: Dict[str, Any]) -> float:
    ...
    for k, penalty in SECURITY_PENALTIES.items():
        if not sec.get(k, False): base -= penalty
    return clamp(base)

def compute_category_performance(perf: Dict[str, Any]) -> float:
    score = 100; ttfb = perf.get('ttfb_ms', 0); size = perf.get('payload_kb', 0); cache = perf.get('cache_max_age_s', 0)
...
class AuditEngine:
    ...
    def compute_metrics(self) -> Dict[int, Dict[str, Any]]:
        ...
        r       = fetch(origin)
        headers = r.headers; content = r.content
        ...
        perf = {
            ...
            'payload_kb': int(len(content)/1024),
        ...
        # Severity placeholders from headers
        errors, warnings, notices = 0, 0, 0
        for k in ['csp','hsts','x_frame','referrer_policy']:
            if not sec[k]: warnings += 1
