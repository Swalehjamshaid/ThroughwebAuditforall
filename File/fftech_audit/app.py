...
app = FastAPI(title="FF Tech AI Website Audit SaaS", version="7.2")
...
def audit_open_ssr(request: Request, url: str = Form(...)):
    ...
    ctx = {"request": request, "url_display": origin,
...
def compute_next_run(hour: int, minute: int, tz_name: str, frequency: str,
                     now_utc: datetime.datetime | None = None) -> datetime.datetime:
    if now_utc is None:
...
def schedule_set(req: Dict[str, str] = Body(...)):
    ...
    try:
        ...
        if not sch:
            sch = Schedule(user_id=user.id, url=origin, frequency=frequency, enabled=True,
                           scheduled_hour=hh, scheduled_minute=mm, timezone=timezone,
        ...
        db.commit()
        return {"message":"Schedule set","schedule_id":sch.id,"next_run_at_utc":sch.next_run_at.isoformat(),
                "url":origin,"frequency":sch.frequency,"time_of_day":f"{hh:02d}:{mm:02d}","timezone":sch.timezone}
    finally:
        db.close()
...
def schedule_list(token: str):
    ...
    try:
        ...
        out = []
        for s in items:
            out.append({"schedule_id": s.id, "url": s.url, "enabled": s.enabled, "frequency": s.frequency,
                        "time_of_day": f"{s.scheduled_hour:02d}:{s.scheduled_minute:02d}",
                        "timezone": s.timezone, "next_run_at_utc": s.next_run_at.isoformat() if s.next_run_at else None})
...
def scheduler_loop():
    while True:
        try:
            db = SessionLocal(); now_utc = datetime.datetime.utcnow().replace(tzinfo=datetime.timezone.utc)
            ...
            for sch in due:
                ...
                sch.next_run_at = compute_next_run(sch.scheduled_hour or 9, sch.scheduled_minute or 0,
                                                   sch.timezone or "UTC", sch.frequency or "weekly",
