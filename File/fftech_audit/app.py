...
# Registered Audit: store history & enforce free quota (10)
@app.post("/audit/user")
def audit_user(req: Dict[str, str] = Body(...), db: Session = Depends(get_db)):
    ...
    if (user.plan or "free").lower() == "free" and user.audits_count >= 10:
        raise HTTPException(status_code=403, detail="Free plan limit reached (10 audits). Upgrade to enable more + scheduling.")
    ...
    db.commit()
    return {"message":"Audit stored", "score": metrics[1]["value"], "grade": metrics[2]["value"], "audit_id": audit.id}
...
def schedule_set(req: Dict[str, str] = Body(...), db: Session = Depends(get_db)):
    token = (req.get("token") or "").strip()
    user = require_user_token(token, db)
    # Allow scheduling only for non-free plans
    if (user.plan or "free").lower() == "free":
        raise HTTPException(status_code=403, detail="Scheduling is a premium feature. Upgrade plan.")
    url = (req.get("url") or "").strip()
