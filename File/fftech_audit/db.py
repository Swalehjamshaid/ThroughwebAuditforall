...
class MagicLink(Base):
    ...
    purpose = Column(String(32), default="verify")
...
def ensure_schedule_columns():
    insp = inspect(engine)
    if "schedules" not in insp.get_table_names(): return
    ...
    with engine.begin() as conn:
        for name, ddl in [
            ...
            if name not in existing:
                ...
                except Exception as e: print("[DB] DDL failed:", ddl, e)

def ensure_user_columns():
    insp = inspect(engine)
    if "users" not in insp.get_table_names(): return
