...
from fastapi import FastAPI, Response, Request, Depends, HTTPException, Form
...
app = FastAPI(title="FF Tech – WebAudit (Zero‑Touch + Complete UI)")
...
# Cookie-based current user for UI
...
async def verify(token: str, request: Request, db: Session = Depends(get_db)):
    ...
    db.add(u); db.commit()
    return RedirectResponse(url='/verify_success')
...
async def ui_login(response: Response, email: str = Form(...), password: str = Form(...), db: Session = Depends(get_db)):
    ...
    # Secure cookie for UI
    response = RedirectResponse(url='/', status_code=303)
    response.set_cookie('auth', token, httponly=True, secure=True, samesite='lax', max_age=60*120)
    return response
