...
app = FastAPI(title="FF Tech – WebAudit v4.3")
...
def startup() -> None:
    ...
    # Fire-and-forget background task (safe because uvicorn/gunicorn manage the loop)
    asyncio.create_task(schedule_loop())

@app.get('/health')
def health() -> dict:
    """Simple health endpoint used by Railway healthcheck."""
...
def absolute_url(request: Request, path: str) -> str:
    """Build absolute URL, aware of Railway's proxy headers."""
    scheme = request.headers.get('x-forwarded-proto') or request.url.scheme
    host = request.headers.get('x-forwarded-host') or request.url.netloc
    return f"{scheme}://{host}{path}"

def ui_current_user(request: Request, db: Session) -> User | None:
    """Return current user from secure cookie, if present."""
...
async def register_submit(
    ...
    # If SMTP not configured and AUTO_VERIFY true, mark verified so onboarding works
    ...
    if smtp_configured():
        ...
        send_email(
            u.email,
            "Verify your FF Tech account",
            f"<p>Click to verify: {verify_link}Verify</a></p>"
        )
...
async def verify(token: str, request: Request, db: Session = Depends(get_db)):
    ...
    u = db.query(User).get(vt.user_id)
    u.verified = True
    db.add(u)
    db.commit()
...
async def ui_login(
    ...
    # Secure, HTTP-only cookie (Railway behind HTTPS → secure=True is fine)
