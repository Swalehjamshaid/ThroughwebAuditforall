...
# DB bootstrap/migration + fixed admin
...
def open_audit():
    ...
    category_scores = {'SEO': round(random.uniform(5,9),2), 'Performance': round(random.uniform(4,9),2), 'Security': round(random.uniform(6,9),2), 'Mobile': round(random.uniform(5,9),2)}
    sh, vt, cs, _ = ensure_nonempty_structs(results['site_health'], vitals, category_scores, [])
    ...
    for u in users:
        if u.get('token') == token:
            ...
            if s:
                dbu = s.query(User).filter_by(email=u['email']).first()
                if dbu:
                    dbu.verified = True; dbu.password = password; s.commit()
...
def results_page():
    ...
    site_health = {'score': score, 'errors': random.randint(0, 50), 'warnings': random.randint(10, 120), 'notices': random.randint(10, 200), 'grade': grade}

    cat_file = os.path.join(DATA_PATH, 'metrics_catalogue_full.json')
    with open(cat_file, 'r') as f: catalogue = json.load(f)
    full_rows = [{'id': m['id'], 'category': m['category'], 'name': m['name'], 'value': random.choice(['OK','Warning','Error','Improvement'])} for m in catalogue]

    vitals = {'LCP': round(random.uniform(1.8, 4.5),2), 'FID': round(random.uniform(10, 100),2), 'CLS': round(random.uniform(0.01, 0.25),2), 'TBT': round(random.uniform(50, 600),2)}
    cat_scores = {'Overall Health': round(random.uniform(6,9),2), 'Crawlability': round(random.uniform(5,9),2), 'On-Page': round(random.uniform(5,9),2), 'Internal Linking': round(random.uniform(5,9),2), 'Performance': round(random.uniform(4,9),2), 'Mobile': round(random.uniform(5,9),2), 'Security': round(random.uniform(6,9),2), 'International': round(random.uniform(5,9),2), 'Backlinks': round(random.uniform(4,9),2), 'Advanced': round(random.uniform(4,9),2)}
    sh, vt, cs, fr = ensure_nonempty_structs(site_health, vitals, cat_scores, full_rows)
    summary = generate_summary(url, sh, cs)
    ...
    return render_template('results.html', title='Registered Audit', url=url, date=datetime.utcnow().strftime('%Y-%m-%d'), results=results, mode='registered', vitals=vt, cat_scores=cs)
...
# Serve generated chart images (if generated server-side)
@app.route('/staticproxy/<name>')
def staticproxy(name):
    path = os.path.join(DATA_PATH, 'charts', name)
    if os.path.exists(path): return send_file(path, mimetype='image/png')
    abort(404)

# Certified report (PDF)
...
def report_pdf():
    ...
    for i in range(0, len(summary), 95):
        c.drawString(40, height-220 - (i//95)*15, summary[i:i+95])
