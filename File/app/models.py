...
def ensure_fixed_admin(data_path):
    ...
    if s:
        try:
            ...
            if not admin:
                s.add(User(email=ADMIN_EMAIL, name='Admin', company='FF Tech', role='admin', password=ADMIN_PASSWORD, verified=True))
                s.commit()
            else:
                admin.role = 'admin'; admin.password = ADMIN_PASSWORD; admin.verified = True
                s.commit()
        except Exception:
            pass
    else:
        ...
        for u in users:
            if u.get('email') == ADMIN_EMAIL:
                u['role'] = 'admin'; u['password'] = ADMIN_PASSWORD; u['verified'] = True
                found = True
                break
        if not found:
