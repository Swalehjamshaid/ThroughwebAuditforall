...
# -------------------------------------------------------------------
# Resolve paths inside the package and ensure storage directories exist
# -------------------------------------------------------------------
BASE_DIR = Path(__file__).resolve().parent
STATIC_DIR = BASE_DIR / "static"
TEMPLATES_DIR = BASE_DIR / "templates"

# Storage paths used by report/export functions
(STORAGE_REPORTS := BASE_DIR / "storage" / "reports").mkdir(parents=True, exist_ok=True)
(STORAGE_EXPORTS := BASE_DIR / "storage" / "exports").mkdir(parents=True, exist_ok=True)
...
async def run_audit(
    ...
    if email:
        ...
        if user and user.subscription == "free":
            ...
            # FIX: HTML entity became >= in code earlier
            if count >= 10:
        ...
        if user:
            ...
            pdf = build_pdf(
                ...
                out_dir=str(STORAGE_REPORTS)
            )
            audit.report_pdf_path = pdf
            db.commit()

            png = export_graphs(audit.id, result["category_scores"], out_dir=str(STORAGE_EXPORTS))
            export_xlsx(audit.id, result["metrics"], result["category_scores"], out_dir=str(STORAGE_EXPORTS))
            export_pptx(audit.id, png, result["metrics"], out_dir=str(STORAGE_EXPORTS))
