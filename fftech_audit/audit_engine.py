...
- Robust HTML parsing with correct regex (no HTML-escaped tokens)
- Transparent scoring: Score, Grade, Category breakdowns
- Safe request wrapper and friendly fallbacks
"""
...
def safe_request(url: str, timeout: int = 10) -> Tuple[int, bytes, float, Dict[str, str]]:
    """Simple HTTP GET with UA header; returns (status_code, content_bytes, latency_seconds, headers_dict)."""
    t0 = time.time()
    try:
...
def safe_search(pattern: str, text: str, flags=0):
    """Run re.search safely; log pattern errors, return None on failure."""
    try:
        return re.search(pattern, text, flags)
    except re.error as e:
        # Print to stderr; the app's log will capture it
...
def register_metrics():
    sections = {
        "A": [
            "Overall Site Health (%)","Website Grade","Executive Summary",
            "Strengths","Weak areas","Priority fixes","Severity (errors, warnings, notices)",
            "Category scores","Industry-standard presentation","Export readiness"
        ],
        "C": [
            "HTTP 2xx","HTTP 4xx","HTTP 5xx","Broken internal links","Broken external links",
            "robots.txt","Sitemap presence","Canonical present","Open Graph/Twitter presence"
        ],
        "D": [
            "Missing title","Title too long","Title too short","Missing meta description",
            "Meta too long","Meta too short","Missing H1","Multiple H1","Missing image alts",
            "URL length","Uppercase in URL"
        ],
        "E": [
            "Total page size KB","Requests per page (approx)","Server response ms","Caching missing",
            "Compression enabled","Render-blocking approx","Approx DOM nodes","3rd-party scripts",
            "Potentially large images","Lazy loading missing"
        ],
        "F": [
            "Viewport meta present","HTTPS enabled","Mixed content","Missing security headers"
        ],
        "X": [
            "Crawlability score","On-Page SEO score","Performance score","Security score","Mobile score",
            "Fix priority","Risk severity","Quick wins","Speed improvement potential",
            "Security improvement potential","Growth readiness"
        ]
    }
    idx = 1
...
class AuditEngine:
    ...
    def _extract(self):
        # Extract href/src references (simple heuristics)
    ...
    def compute_metrics(self) -> Dict[int, Dict[str, Any]]:
        ...
        m[64] = {"value": 1 if re.search(r"[A-Z]", self.url) else 0,
                 "detail": "Uppercase in URL" if re.search(r"[A-Z]", self.url) else "Lowercase"}
        is_https = self.url.startswith("https://")
        m[105] = {"value": 1 if is_https else 0, "detail": "HTTPS enabled" if is_https else "Not HTTPS"}
        total_errors += 0 if is_https else 1

        mixed = any(link.startswith("http://")
                    for link in (self.links_internal + self.resources_js + self.resources_css + self.resources_img)) and is_https
        ...
        # ---------- Viewport / Canonical / Social tags ----------
        viewport_meta = safe_search(r'<meta[^>]+name=["\']viewport["\']', self.html, flags=re.IGNORECASE)
        m[98] = {"value": 1 if bool(viewport_meta) else 0,
                 "detail": "Viewport meta present" if viewport_meta else "Missing viewport"}
        total_warnings += 0 if viewport_meta else 1

        # **FIXED canonical regex** (correct < > and complete char class)
        ...
        og_or_twitter = bool(
            safe_search(r'property=["\']og:',    self.html, flags=re.IGNORECASE) or
            safe_search(r'name=["\']twitter:',   self.html, flags=re.IGNORECASE)
        )
        m[62] = {"value": 0 if og_or_twitter else 1,
                 "detail": "Open Graph/Twitter present" if og_or_twitter else "Missing"}

        # ---------- robots/sitemap ----------
        robots_url = f"{urlparse(self.url).scheme}://{self.domain}/robots.txt"
        rcode, rcontent, _, _ = safe_request(robots_url)
        m[29] = {"value": 0 if rcode == 200 and rcontent else 1,
                 "detail": "robots.txt present" if rcode == 200 else "robots.txt missing"}
        ...
        for path in ["/sitemap.xml","/sitemap_index.xml","/sitemap"]:
            ...
            if scode == 200 and scontent:
                ...
                break
        m[136] = {"value": 1 if sitemap_present else 0,
                  "detail": "Sitemap present" if sitemap_present else "Sitemap missing"}
        ...
        m[85] = {"value": len(self.resources_css)+len(self.resources_js)+len(self.resources_img),
                 "detail": "Requests per page (approx)"}
        ...
        m[94] = {"value": 0 if "max-age" in cache_control.lower() else 1,
                 "detail": f"Cache-Control: {cache_control}"}
