...
# fftech_audit/app.py
"""
FF Tech AI Website Audit SaaS — Main Application
- Fixes Railway healthcheck (adds /health; binds to 0.0.0.0:$PORT)
- Uses robust scheduler loop (safe session contexts; commit/rollback; controlled sleep)
"""
...
app = FastAPI(title=APP_NAME, version="3.1.0", description="Main entrypoint integrating DB, Auth, Metrics, UI & PDF")
app.add_middleware(CORSMiddleware, allow_origins=["*"], allow_methods=["*"], allow_headers=["*"])
security = HTTPBearer()

# Ensure DB tables exist (columns added via db.ensure_schedule_columns)
...
async def scheduler_loop():
    """
    Run due schedules, save audits, reschedule next_run_at, and email PDF.
    Uses safe session context to avoid pool exhaustion; handles commit/rollback.
    """
    while True:
        try:
                ...
                for s in due:
                    ...
                    try:
                        ...
                        try:
                            ...
                            # Email failures should not stop scheduler
                            print(f"[Scheduler] Email send failed for {s.url}: {e}")
