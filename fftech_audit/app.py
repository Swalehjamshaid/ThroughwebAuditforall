...
def landing(request: Request):
    return templates.TemplateResponse("index.html", {"request": request, "build_marker": "v2025-12-28-SSR-7"})
...
async def audit_open_ssr(request: Request, url: str = Form(...)):
    ...
    # ✅ JSON-stringify dict/list values so Jinja prints them cleanly
    ...
    return templates.TemplateResponse(
        "results.html",
        {
            ...
            "rows": rows,
            "allow_pdf": False,
            "build_marker": "v2025-12-28-SSR-7",
        },
    )
...
def auth_register(request: Request,
    ...
    # Sends email if SMTP configured; logs DEV verify link otherwise
...
def schedule_set(payload: Dict[str, str] = Body(...), user: User = Depends(require_user), db: Session = Depends(get_db)):
    ...
    try:
        run_at = datetime.datetime.fromisoformat(run_at_iso.replace("Z", "+00:00")) if run_at_iso else (
            now_utc() + datetime.timedelta(days=7)
        )
    except Exception:
    ...
    if not sch:
        sch = Schedule(user_id=user.id, url=url, frequency=freq, enabled=True, next_run_at=run_at)
        db.add(sch)
    else:
        sch.frequency = freq
        sch.enabled = True
        sch.next_run_at = run_at
