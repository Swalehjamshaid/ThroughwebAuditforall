...
# Static mount (fallback to CDN if static missing)
...
def ctx_base(request: Request) -> Dict[str, Any]:
    return {"request": request, "ASSETS": assets(), "build_marker": "v2025-12-28-SSR-7"}
...
async def audit_open_ssr(request: Request, url: str = Form(...)):
    ...
    ctx = ctx_base(request); ctx.update({
...
def report_pdf_api(req: Dict[str, str], user: User = Depends(require_user), db: Session = Depends(get_db)):
    url = req.get("url"); if not url or not is_valid_url(url): raise HTTPException(status_code=400, detail="Invalid URL")
    eng = AuditEngine(url); metrics = eng.compute_metrics()
    ...
    return StreamingResponse(io.BytesIO(pdf), media_type="application/pdf", headers={"Content-Disposition": 'attachment; filename="FFTech_Audit.pdf"'})
...
def schedule_set(payload: Dict[str, str] = Body(...), user: User = Depends(require_user), db: Session = Depends(get_db)):
    url = (payload.get("url") or "").strip(); freq = (payload.get("frequency") or "weekly").strip().lower()
    ...
    while True:
        try:
            ...
            for sch in due:
                ...
                db.add(audit); db.commit()
                pdf = build_pdf_report(audit, metrics)
                send_email_with_pdf(user.email, "Your FF Tech Audit Report", f"Attached: 5-page audit report for {sch.url}.", pdf, "FFTech_Audit.pdf")
                sch.next_run_at = now + (datetime.timedelta(days=1) if sch.frequency == "daily" else datetime.timedelta(days=7)); db.commit()
            db.close()
        except Exception as e:
