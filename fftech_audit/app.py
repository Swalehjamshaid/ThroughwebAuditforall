...
class AuditEngine:
    ...
    def compute_metrics(self) -> Dict[int, Dict[str, Any]]:
        ...
        missing_alts = sum(1 for tag in img_tags if re.search(r'\salt=["\'][^"\']*["\']', tag, flags=re.IGNORECASE) is None)
        ...
        # ---------- Canonical URL (FULLY FIXED) ----------
        canonical_match = safe_search(
            r'<link[^>]+rel=["\']canonical["\'][^>]+href=["\']([^"\']+)["\']',
            self.html,
            flags=re.IGNORECASE
        )
        canonical_href = canonical_match.group(1).strip() if canonical_match else None
        ...
        m[94] = {"value": 0 if "max-age" in cache_control else 1,
                 "detail": f"Cache-Control: {cache_control or 'none'}"}
    ...
    def executive_summary(self, metrics: Dict[int, Dict[str, Any]]) -> str:
        ...
        return (
            ...
            f"Approx payload {perf:.1f} KB; server response {resp:.0f} ms. Strengths: {strengths}. Weaknesses: {weaknesses}. "
